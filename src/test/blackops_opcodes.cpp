...
BOOST_FIXTURE_TEST_SUITE(blackops_opcodes_tests, BasicTestingSetup)
...
static void CheckTestResultForAllFlags(const stacktype &original_stack,
    ...
    for (uint32_t flags : flagset) {
        ...
        bool r =
            EvalScript(stack, script, flags | SCRIPT_ENABLE_BLACKOPS_OPCODES, 
                       sigchecker, &err);
        BOOST_CHECK(r);
        BOOST_CHECK(stack == expected);

        // Make sure that if we do not pass the blackops flag, opcodes are still
        // disabled.
...
static void CheckError(uint32_t flags, const stacktype &original_stack,
    ...
    ScriptError err = SCRIPT_ERR_OK;
    stacktype stack{original_stack};
    bool r = EvalScript(stack, script, flags | SCRIPT_ENABLE_BLACKOPS_OPCODES, 
                       sigchecker, &err);
    BOOST_CHECK(!r);
