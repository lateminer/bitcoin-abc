...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity) {
    ...
    for (size_t i = 0; i < sizeof(blockinfo) / sizeof(*blockinfo); ++i) {
        ...
        BOOST_CHECK(ProcessNewBlock(config, shared_pblock, true, nullptr, pblock->GetHash()));
...

    {
        ...
        BOOST_CHECK(ContextualCheckTransactionForCurrentBlock(
            config, CTransaction(tx), state, 0));
    }

    // Sequence locks fail.
    BOOST_CHECK(!TestSequenceLocks(CTransaction(tx), 0));
    // Sequence locks pass on 2nd block.
    BOOST_CHECK(
        SequenceLocks(CTransaction(tx), 0, &prevheights,
