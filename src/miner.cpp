...
int64_t UpdateTime(CBlock *pblock, const Config &config,
    ...
    int64_t nNewTime =
        std::max(pindexPrev->GetPastTimeLimit() + 1, GetAdjustedTime());
    ...
    if (consensusParams.fPowAllowMinDifficultyBlocks) {
        pblock->nBits = GetNextTargetRequired(pindexPrev, pblock, pblock->IsProofOfStake(), config);
    }
...
BlockAssembler::CreateNewBlock(const CScript &scriptPubKeyIn, Amount *pFees, bool fProofOfStake) {
    ...
    if (!fProofOfStake) {
        coinbaseTx.vout[0].nValue =
            nFees + GetProofOfWorkSubsidy(nHeight, chainparams.GetConsensus());
...
void ThreadStakeMiner(CWalletRef &pwallet, const CChainParams& chainparams)
{
    ...
    while (true)
    {
        ...
        if(pwallet->HaveAvailableCoinsForStaking())
        {
            ...
            std::unique_ptr<CBlockTemplate> pblocktemplate(BlockAssembler(GetConfig()).CreateNewBlock(reservekey.reserveScript, &nTotalFees, true));
            if (!pblocktemplate.get())
                return;
            CBlockIndex* pindexPrev =  chainActive.Tip();
            ...
            for(uint32_t i=beginningTime;i<beginningTime + MAX_STAKE_LOOKAHEAD;i+=stakeTimeMask+1) {
                ...
                if (SignBlock(pblock, pwallet, nTotalFees, i)) {
                    ...
                    std::unique_ptr<CBlockTemplate> pblocktemplatefilled(
                            BlockAssembler(GetConfig()).CreateNewBlock(pblock->vtx[1]->vout[1].scriptPubKey, &nTotalFees,
                                                                    true));
                    ...
                    if (SignBlock(pblockfilled, pwallet, nTotalFees, i)) {
                        ...
                        if(validBlock) {
                            CheckStake(pblockfilled, *pwallet, GetConfig(), *pcoinsTip);
